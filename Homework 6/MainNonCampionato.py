...
OUT_DIR = "data/full_dataset_deduped" # Ho cambiato nome cartella per distinguere
...
# ============================================================
# 1. DATASET BUILD (CON DEDUPLICAZIONE ONE-TO-ONE)
# ============================================================
...
def stream_and_filter(path, vins, mapping):
    ...
    for chunk in pd.read_csv(path,
        ...
        # Normalizzazione VIN al volo per il filtro
        chunk[vin_col] = chunk[vin_col].astype(str).str.strip().str.upper()
        
        # Filtro: teniamo solo le righe con VIN nell'intersezione
        chunk = chunk[chunk[vin_col].isin(vins)]
        
        if chunk.empty:
            continue

        # Pulizia minima per non rompere il codice dopo
...
def build_dataset():
    ...
    # Qui usiamo i nomi colonne originali dei CSV
    ...
    print(f"▶ Trovati {len(common_vins)} VIN in comune. Estrazione dati...")

    target_vins = set(common_vins)

    # Estrazione (potrebbe contenere duplicati se un VIN appare più volte nel file)
    ...
    print(f"✔ Dataset DEDUPLICATO creato con {len(final_vins)} coppie uniche.")
...
def run_record_linkage(df1, df2, strategy):
    ...
    # Assicuriamo che le colonne di blocco siano stringhe pulite per evitare crash
...
def run_dedupe_B1(linker, c_data, u_data, threshold=0.5):
    ...
    pairs = []
    # FIX: Gestione sicura se 'make' è None
...
def export_ditto_data(df_gt, df1, df2, filename):
    ...
    # 2. Negativi
    n_pos = len(lines)
    all_id1 = df1.index.tolist() # Gli indici nel dataframe sono allineati a orig_cl_id se non resettati
    ...
    while count < n_pos:
        ...
        # Accesso sicuro tramite il dataframe originale
        # Nota: Qui assumiamo che df1 abbia indice 0..N, altrimenti usare .iloc
        try:
            vin1 = df1.loc[id1, "vin"]
            vin2 = df2.loc[id2, "vin"]
...
def main():
    ...
    # Resettiamo l'indice per avere ID puliti da 0 a N
    ...
    # Ground truth (VIN)
    # Poiché abbiamo deduplicato, ci aspettiamo un rapporto 1:1 quasi perfetto
