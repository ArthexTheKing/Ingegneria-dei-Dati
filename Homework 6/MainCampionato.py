...
CRAIGSLIST_RAW = "data/dataset/vehicles.csv"
USEDCARS_RAW = "data/dataset/used_cars_data.csv"

# Cartella dedicata per il dataset 1-to-1
OUT_DIR = "data/part_dataset_deduped"
CRAIGSLIST_SAMPLE = f"{OUT_DIR}/vehicles.csv"
USEDCARS_SAMPLE = f"{OUT_DIR}/used_cars_data.csv"
DITTO_TR = f"{OUT_DIR}/ditto_train.txt"
DITTO_VA = f"{OUT_DIR}/ditto_val.txt"
DITTO_TE = f"{OUT_DIR}/ditto_test.txt"

# --- PARAMETRI CAMPIONAMENTO ---
TARGET_FINAL_SIZE = 2000      # Quante coppie vogliamo alla fine
CANDIDATE_POOL_SIZE = 15_000  # Quanto cercare prima di filtrare
...
# ============================================================
# 1. DATASET BUILD (SAMPLED + DEDUPLICATED)
# ============================================================
...
def get_vins_lightweight(path, vin_col):
    ...
    for chunk in pd.read_csv(path, usecols=[vin_col],
                             ...
                             low_memory=False):
        vins.update(chunk[vin_col].dropna().astype(str).str.strip().str.upper())
    return vins
...
def stream_and_filter(path, vins, mapping):
    ...
    for chunk in pd.read_csv(path,
        ...
        chunk[vin_col] = chunk[vin_col].astype(str).str.strip().str.upper()
...
def build_dataset():
    if os.path.exists(CRAIGSLIST_SAMPLE) and os.path.exists(USEDCARS_SAMPLE):
        if os.path.exists(CRAIGSLIST_SAMPLE): os.remove(CRAIGSLIST_SAMPLE)
        if os.path.exists(USEDCARS_SAMPLE): os.remove(USEDCARS_SAMPLE)
        if os.path.exists(DITTO_TR): os.remove(DITTO_TR)
        if os.path.exists(DITTO_TE): os.remove(DITTO_TE)
        if os.path.exists(DITTO_VA): os.remove(DITTO_VA)
    ...
    # 1. CAMPIONAMENTO INIZIALE (Pool)
    # Se abbiamo più VIN del pool (es. 15k), ne prendiamo un sottoinsieme a caso
    pool_size = min(len(common_vins), CANDIDATE_POOL_SIZE)
    print(f"▶ Trovati {len(common_vins)} VIN comuni. Estrazione pool di {pool_size}...")
    
    sample_vins = set(random.sample(common_vins, pool_size))

    df_cl = stream_and_filter(CRAIGSLIST_RAW, sample_vins, CRAIGSLIST_MAPPING)
    df_uc = stream_and_filter(USEDCARS_RAW, sample_vins, USEDCARS_MAPPING)

    # 2. DEDUPLICAZIONE (One-to-One)
    print(f"   Righe estratte (con duplicati): CL={len(df_cl)}, UC={len(df_uc)}")
    
    df_cl = df_cl.drop_duplicates(subset="VIN", keep="first")
    df_uc = df_uc.drop_duplicates(subset="vin", keep="first")
    
    print(f"   Righe uniche (One-to-One): CL={len(df_cl)}, UC={len(df_uc)}")

    # 3. TAGLIO FINALE AL TARGET (es. 1000)
    # Calcoliamo l'intersezione effettiva rimasta dopo le pulizie
    available_vins = list(set(df_cl["VIN"]).intersection(df_uc["vin"]))
    
    if len(available_vins) > TARGET_FINAL_SIZE:
        print(f"✂️ Campionamento Finale: Riduzione da {len(available_vins)} a {TARGET_FINAL_SIZE} coppie.")
        final_vins = random.sample(available_vins, TARGET_FINAL_SIZE)
    else:
        print(f"ℹ️ Disponibili solo {len(available_vins)} coppie (Target era {TARGET_FINAL_SIZE}). Le uso tutte.")
        final_vins = available_vins

    # Filtro finale sui dataframe
    ...
    print(f"✔ Dataset creato con {len(final_vins)} coppie uniche (1-to-1).")
...
def train_dedupe(df1, df2, train_gt):
    ...
    c_data_full = {f"c_{r.orig_cl_id}": {"make": safe(r.make), "model": safe(r.model), "year": safe(r.year), "price": safe_price(r.price_num)} for _, r in df1.iterrows()}
    u_data_full = {f"u_{r.orig_uc_id}": {"make": safe(r.make), "model": safe(r.model), "year": safe(r.year), "price": safe_price(r.price_num)} for _, r in df2.iterrows()}
...
def serialize_row(row):
    ...
    for col, val in row.items():
        if col not in ignore: items.append(f"COL {col} VAL {val}")
    return " ".join(items)

def export_ditto_data(df_gt, df1, df2, filename):
    print(f"   Exporting {filename}...")
    lines = []
    d1 = df1.set_index("orig_cl_id")
    d2 = df2.set_index("orig_uc_id")

    for _, row in df_gt.iterrows():
        try:
            r1 = d1.loc[row["orig_cl_id"]]
            r2 = d2.loc[row["orig_uc_id"]]
...
def eval_metrics(pairs, gt):
    ...
    print(f"    (Dettaglio: TP={tp}, FP={fp}, FN={fn})")
    return p, r, f1
...
def main():
    ...
    print(f"▶ Ground Truth Totale: {len(gt)} coppie (One-to-One)")
    ...
    print("\n--- Export Ditto ---")
    export_ditto_data(train_gt, cl, uc, DITTO_TR)
    export_ditto_data(val_gt, cl, uc, DITTO_VA)
    export_ditto_data(test_gt, cl, uc, DITTO_TE)
    print("✔ File generati.")
    ...
    for b in ["B1", "B2"]:
        ...
        print(f"RL-{b} | P={p:.2f} R={r:.2f} F1={f1:.2f} Time={time.time()-t0:.3f}s")
    ...
    print(f"Dedupe-B1 | P={p:.2f} R={r:.2f} F1={f1:.2f} Train={train_time:.2f}s Inf={time.time()-t0:.3f}s")

    t0 = time.time()
    pairs_B2 = run_dedupe_B2(linker, c_data, u_data)
    p, r, f1 = eval_metrics(pairs_B2, test_gt)
    print(f"Dedupe-B2 | P={p:.2f} R={r:.2f} F1={f1:.2f} Train={train_time:.2f}s Inf={time.time()-t0:.3f}s")
