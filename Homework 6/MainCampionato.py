...
TARGET_FINAL_SIZE = 2000
CANDIDATE_POOL_SIZE = 15_000
...
def build_dataset():
    ...
    print("🔄 Ricreazione dei dataset in corso...")
    ...
    sample_vins = set(
        random.sample(
            common_vins,
            min(len(common_vins), CANDIDATE_POOL_SIZE)
        )
    )
    ...
    final_vins = list(set(df_cl["VIN"]).intersection(df_uc["vin"]))[:TARGET_FINAL_SIZE]
